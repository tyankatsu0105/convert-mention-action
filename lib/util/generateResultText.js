...
exports.generateResultText = (users, token, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    switch (eventName) {
        ...
        case 'pull_request':
            if (action === 'opened') {
                ...
                // PR レビュー申請
                includeMensionText = _1.generateGitHubMentionTextFromPullRequest(pull_request);
                emoji = ':bell:';
    ...
    const members = _1.getMatchedMembers(yield _1.getMembers(token), users, includeMensionText);
    const ids = members.map((member) => member.id);
    const resultText = `${repoName} ${_1.generateSlackMension(ids)}
  ${emoji} ${explain} ${link}`;
    return resultText;
});
